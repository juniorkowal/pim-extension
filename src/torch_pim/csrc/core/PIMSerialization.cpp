...
struct PIMBackendMetadata : public c10::BackendMeta {
  ...
  PIMBackendMetadata(int backend_version_format, int format_number) :
...
void for_serialization(const at::Tensor& t, std::unordered_map<std::string, bool>& m) {
  ...
  auto tmeta = dynamic_cast<PIMBackendMetadata*>(t.unsafeGetTensorImpl()->get_backend_meta());
...
void for_deserialization(const at::Tensor& t, std::unordered_map<std::string, bool>& m) {
  ...
  c10::intrusive_ptr<c10::BackendMeta> new_tmeta{
      ...
      new PIMBackendMetadata(backend_version_format, format_number))
    };
...
bool check_backend_meta(const at::Tensor& t) {
  if (t.unsafeGetTensorImpl()->get_backend_meta_intrusive_ptr()) {
    PIMBackendMetadata* tmeta = dynamic_cast<PIMBackendMetadata*>(t.unsafeGetTensorImpl()->get_backend_meta());
