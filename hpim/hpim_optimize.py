...
from .ops import pim_matmul, pim_add, pim_transpose


class OptimizerContextManager:
    def __init__(self):
        self.torch_matmul = torch.matmul

    def __enter__(self):
        torch.matmul = pim_matmul
        print("patched matmul with hpim")
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        torch.matmul = self.torch_matmul
        print("restored original torch.matmul")

def optimize():
    return OptimizerContextManager()